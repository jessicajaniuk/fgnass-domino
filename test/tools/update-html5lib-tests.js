...
var list_tests = function () {
  ...
  var testfiles = fs
    ...
    .map(function (f) {
      return path.normalize(path.join(base, f));
    });
  testfiles.sort();
  return testfiles;
};
...
var parse_one_test = function (filename, testcase) {
  ...
  var fragment = m[4] ? { name: localname(m[4]), ns: namespace(m[4]) } : undefined;
  return {
    //file: filename,
    data: m[1] || '',
    errors: m[3].split(/\n/g).slice(0, -1),
...
var serialize_doc = function (filename, fragment, doc) {
  ...
  var pop_stack = function () {
      ...
      if (old.ns === namespace('html') && EMPTY[old.tag]) {
        if (old.children.length > 0) {
          throw new ParseError(
            'Empty elements (' + old.tag + ") can't have children",
            filename,
            doc
          );
        }
    ...
    if (old.attrs && old.attrs.length === 0) {
      old.attrs = undefined;
    }
    return old;
  };
  ...
  while (doc.length > 0) {
    ...
    if (attrname !== undefined) {
      if (!can_add_attr) throw new ParseError('Late attribute', filename, m);
      obj = {
    ...
    if (text !== undefined) {
      obj = { text: text };
      if (stack_top().ns === namespace('html') && NO_ESCAPE[stack_top().tag]) {
        obj.no_escape = props.no_escape = true;
      }
      if (
        stack_top().ns === namespace('html') &&
...
var twiddle_test = function (filename, tc) {
  ...
  if (filename === 'tests23' && /size=4 id=a/.test(tc.data) && /id="a" size="4"/.test(expected)) {
    expected = expected.replace(/(id=[^> ]+) (size=[^> ]+)/g, '$2 $1');
  }
  if (filename === 'tests26' && /<code code="" x<="">/.test(expected)) {
    expected = expected.replace(/(code=[^> ]+) (x<=[^> ]+)/g, '$2 $1');
  }
  if (filename === 'webkit01' && /<rdar: 6869687="" problem="">/.test(expected)) {
